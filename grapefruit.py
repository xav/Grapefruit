...
class Color:
    '''Hold of a color value.
    
    The class stores both the RGB and HSV representation of the color to keep
    the specified hue during the manipulations when the color is a pure white
    due to its lightness.
    
    Two constant properties are available:
        * WHITE_REFERENCE:
            The CIE white reference points used for the L*a*b* conversions
        * NAMED_COLOR:
            The X11 color names and RGB values.
    ...
    the results are not capped. This is due to some color spaces being 'wider'
    than others.
    
    The class instances are immutable, all the methods return a new instance
    of the grapeFruit.Color class, and all the properties are read-only.
    
    Additionally, static metods are available to convert colors stored as
    value lists instead of grapefruit.Color instances.
    
    Note:
     - For the manipulation functions, the white reference of the resulting
       color is the one of the calling instance (except for
       Color.ColorWithWhiteRef, obviously).
     - Nearly all the instance properties return calculated values (except
       for alpha, whiteRef, rgb and hsl). Those values are not cached and
       will thus be recalculated each time you access the property.
    ...
    '''
    The reference white points of the CIE standards illuminants.
    
    Contains the reference white points required for the XYZ>->LAB
    transformation, for all the the standard illuminants.
    
    The values were calculated from the chromaticity coordinates found at:
    http://en.wikipedia.org/wiki/Standard_illuminant
    
    The key names are build using the following pattern:
        observer_illuminant
    ...
    The possible values for illuminant are the name of the standard
    illuminants:
        ...
        - E:   ~D@5455 K    Equal energy radiator (not a black body)
    ...
    A list of the X11 colors supported by popular browsers.
    
    The gray/grey spelling issues is fixed so that both versions work
    (e.g lightgrey or lightgray).
    ...
    def __init__(self, values, mode='rgb', alpha=1.0, wref=_DEFAULT_WREF):
        ...
        Args:
            values: The values of this color, in the specified representation.
            mode: The representation mode used for values. [optional]
            alpha: the alpha value (transparency) of this color.
    ...
    def RgbToHsl(r, g, b):
        ...
        Returns:
            A tuple containing the (h, s, l) representation of the color in
            the following range:
            h: [0~360]
            s: [0~1]
            l: [0~1]
    ...
    def HslToRgb(h, s, l):
        ...
        Returns:
            A tuple containing the (r, g, b) representation of the color in
            the following range:
            r: [0~1]
            g: [0~1]
            b: [0~1]
    ...
    def RgbToHsv(r, g, b):
        ...
        Returns:
            A tuple containing the (h, s, v) representation of the color in
            the following range:
            h: [0~360]
            s: [0~1]
            v: [0~1]
    ...
    def RgbToYiq(r, g, b):
        ...
        Returns:
            A tuple containing the (y, i, q) representation of the color in
            the following range:
            y: [0~1]
            i: [0~1]
            q: [0~1]
    ...
    def RgbToYuv(r, g, b):
        ...
        Returns:
            A tuple containing the (y, u, v) representation of the color in
            the following range:
            y: [0~1]
            u: [-0.436~0.436]
            v: [-0.615~0.615]
    ...
    def RgbToXyz(r, g, b):
        ...
        Returns:
            A tuple containing the (x, y, z) representation of the color in
            the following range:
            x: [0~1]
            y: [0~1]
            z: [0~1]
    ...
    def XyzToLab(x, y, z, wref=_DEFAULT_WREF):
        ...
        Returns:
            A tuple containing the (L, a, b) representation of the color in
            the following range:
            L: [0~100]
            a: [-1~1]
            b: [-1~1]
    ...
    def LabToXyz(l, a, b, wref=_DEFAULT_WREF):
        ...
        Returns:
            A tuple containing the (x, y, <) representation of the color in
            the following range:
            x: [0~q]
            y: [0~1]
            z: [0~1]
    ...
    def CmykToCmy(c, m, y, k):
        ...
        Returns:
            A tuple containing the (c, m, y) representation of the color in
            the following range:
            c: [0~1]
            m: [0~1]
            y: [0~1]
    ...
    def CmyToCmyk(c, m, y):
        ...
        Returns:
            A tuple containing the (c, m, y, k) representation of the color in
            the following range:
            c: [0~1]
            m: [0~1]
            y: [0~1]
    ...
    def HtmlToRgb(html):
        ...
        Args:
            html:
                the HTML definition of the color (#RRGGBB or #RGB or
                a color name)
    ...
    def _WebSafeComponent(c, alt=False):
        ...
        Args:
            c: the component value [0~1]
            alt:
                if True, return the alternative value instead
                of the nearest one.
    ...
    def __GetCMYK(self):
        return Color.CmyToCmyk(*RgbToCmy(*self.__rgb))
    ...
    def ColorWitgAlpha(self, alpha):
        ...
        >>> Color.NewFromRgb(1.0, 0.5, 0.0, 1.0).ColorWitgAlpha(0.5)
        (1.0, 0.5, 0.0, 0.5)
    ...
    def ColorWithWhiteRef(self, wref, labAsRef=False):
        ...
        Args:
            ...
            labAsRef:
                If True, the L*a*b* values of the current instance are used
                as reference for the new color;
                otherwise, the RGB values are used as reference.
    ...
    def ColorWithSaturation(self, saturation):
        ...
        Note: The saturation is defined in the HSL mode.
    ...
    def DarkerColor(self, level):
        ...
        Args:
            level:
                the amount by which the color should be darkered to produce
                the new one [0~1].
    ...
    def LighterColor(self, level):
        ...
        Args:
            level: the amount by which the color should be lightened to
            produce the new one [0~1].
    ...
    def TriadicScheme(self, angle=120):
        ...
        Args:
            angle: the angle between the hues of the complimentary colors and
            the of the created colors, the default value makes a regular triad.
